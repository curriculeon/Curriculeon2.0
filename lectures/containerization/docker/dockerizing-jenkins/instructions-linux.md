...
### Create Directory
* Create a new directory from `Terminal`
...
* Each flag from the above command is described more thoroughly below
    ...
    * `~/Docker/Jenkins` - my created directory
...
* The `/var/jenkins_home/secrets/initialAdminPassword` file is located under mounted place.
    * For example if `/var/jenkins_home` is "mapped" to `~/Docker/Jenkins`, then file is located at `~/Docker/Jenkins/secrets/initialAdminPassword`
