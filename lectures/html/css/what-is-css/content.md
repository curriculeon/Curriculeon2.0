...
#### External Style Sheet
* external stylesheets stylize the page by referencing css written in a separate file with a `.css` extension
    * include a reference to the external style sheet file inside the `<link>` element.
    * The `<link>` element is placed in the `<head>` section by convention
...
#### Internal Style Sheet
* internal stylesheets stylize the page by referencing css written in the same file in the `<head>` tag.
